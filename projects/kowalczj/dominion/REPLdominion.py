    ...
    def play(self, player, players, supply, trash):
        while "coin" in catinlist(player.hand):
            ...
            if c:
                ...
                player.hand.remove(c)
                while (
                    c.cost > 2 or len(supply["Copper"]) > 0 or len(supply["Silver"]) > 0
                ):
                    pick = input(
                        "Choose a coin to gain.  Any coin up to "
                        + str(c.cost + 3)
                        + ": "
                    )
    ...
    def play(self, player, players, supply, trash):
        while (player.deck or player.discard) and len(player.hand) < 7:
            ...
            if player.hold[-1].category == "action" and not player.yesnoinput(
                "You drew " + player.hold[-1].name + ".  Would you like to keep it?",
                ", add to my hand",
                ", set it aside",
            ):
            ...
            if (not player == this_player) and ("victory" in catinlist(player.hand)):
                for c in player.hand:
                    ...
                    while True:
                        ...
                        c = getcard(
                            putback, supply, player.hand, "your hand", ["victory"]
                        )
                        if c:
                            player.hand.remove(c)
        ...
        for player in players:
            for c in player.hand:
                ...
                if not b:
                    ...
                    print(
                        "The first card in the deck of " + player.name + " is " + b.name
                    )
...
class Player:
    ...
    def turn(self, players, supply, trash):
        ...
        while self.buys > 0:
            ...
            print(buy_string)
            purchase = input(
                "What would you like to purchase?  -Hit enter for no purchase.-\n--> "
            )
            if not purchase:
                break
            else:
    ...
    def gaincard(self, supply, upto):
        while True:
            gain = input(
                "What would you like to get?  Any card up to " + str(upto) + "\n--> "
            )
...
class ComputerPlayer(Player):
    ...
    def getcard(
        ...
        categories=["action", "coin", "curse", "victory"],
        upto=100,
    ):
